...
Oath.prototype.get = function (property, success, failure) {
  var context = this._oath.context;

  if (success) {
    this._register('resolve', function (result) {
      var val = result[property] || null;
      success.call(context, val);
    });
  }

  if (failure) {
    this._register('reject', function (result) {
      var val = result[property] || null;
      failure.call(context, val);
    });
  }
